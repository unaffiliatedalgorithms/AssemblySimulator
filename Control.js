/*
This script needs to come after Assembly.js script as this script uses global variables from that script
*/

/* There are various functions we want to to be executed/updated 
whenever the Enter button is pressed*/
...
// This function is such that mobile website also can have similar functionality to the code above
// we use focus out when we enter text into a textfield and then click somewhere else on the page.
function create_focus_listener(id,func){
	document.getElementById(id).addEventListener("focusout",func);
}

// Once again, but for buttons
function create_button_listener(id,func){
	document.getElementById(id).addEventListener("click",func);
}

// All the things that should happen when the assemble button is pressed
...
// perform a single iteration or pause the current running program
function press_step(){
	if(_start==0){
		iterate();
	}
...
// run in constant mode or pause the simulation if it is running
function press_run(){
	if(_start==0){
...
// result of changing the clock speed
function set_speed(){
...
// Default text to be displayed in the code editor
...
// Initial RAM block. This requires a dynamic preparation of element sizes, if the table size should be able to be adopted to fit different sizes.
function create_ram_table(mat,ncol){
	var elem = document.getElementById('ram');
	var row;
	for(var i=0;i<mat.length;i++){
		if(i%ncol==0){
			// creating a new row if necessary column elements
		...
		// we can use the id generated here for quicker updates later on
		col.setAttribute('id',"ram_"+(i).toString());
		// here we guaranty that each byte is made up of exactly two hexadecimal digits
		var txt = document.createTextNode(("00"+mat[i].toString(16)).toUpperCase().slice(-2));
		col.appendChild(txt);
		row.appendChild(col);
		if(i%ncol==0){
...
function fill_label_table(lookuptable,memory){
	...
	for(var i=0;i<sorted_keys.length; i++){
		...
		var content = ("00"+lookuptable.get(sorted_keys[i]).toString(16)).toUpperCase().slice(-2);
		...
		col.style.width = (100/3).toString()+"%";
		col.setAttribute('class','byte');
		content = ("00"+memory[lookuptable.get(sorted_keys[i])].toString(16)).toUpperCase().slice(-2);
		var txt = document.createTextNode(content);
		col.appendChild(txt);
		row.appendChild(col);
...
function memory_screen(val,loc){
	var txt = ("00"+val.toString(16)).toUpperCase().slice(-2);
